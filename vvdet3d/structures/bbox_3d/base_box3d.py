...
class BaseInstance3DBoxes:
    ...
    def in_range_3d_without_self(
            self, box_range: Union[Tensor, np.ndarray,
                                   Sequence[float]]) -> Tensor:
        """Check whether the boxes are in the given range and remove self.
        ...
        not_self_flags = ((torch.abs(gravity_center[:, 0]) > 1e-3)
                             & (torch.abs(gravity_center[:, 1]) > 1e-3))
        in_range_flags = in_range_flags & not_self_flags
        return in_range_flags

...
class BaseDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.len = 0
        assert ego_vehicle is not None, "`ego_vehicle` must be specified."
    ...
    def load_frame_data(self, scene_idx, timestamp_idx):
        if self.serialize_data:
            start_addr = 0 if scene_idx == 0 else self.data_address[scene_idx - 1].item()
            end_addr = self.data_address[scene_idx].item()
            bytes = memoryview(
                self.data_bytes[start_addr:end_addr])  # type: ignore
            data_info = pickle.loads(bytes)  # type: ignore
        else:
            data_info = copy.deepcopy(self.data_list[scene_idx])
        return data_info[timestamp_idx]
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        for scene_idx, scene_len_cum in enumerate(self.scene_len_list):
            if scene_len_cum > idx:
                break
        timestamp_idx = idx if scene_idx == 0 else \
            idx - self.scene_len_list[scene_idx - 1]
        timestamp_delay_idx = timestamp_idx
        if self.async_mode == 'constant':
            # TODO: test
            timestamp_delta_idx = math.ceil(0 / self.frame_gap)
            timestamp_delay_idx = int(max(0, timestamp_idx - timestamp_delta_idx))


        # data_info = {'now': None, 'past': None}
        # data_info['now'] = self.load_frame_data(scene_idx, timestamp_idx)
        # data_info['past'] = self.load_frame_data(scene_idx, timestamp_delay_idx)
        # data_info['time_delay'] = (timestamp_idx - timestamp_delay_idx) * self.frame_gap
        # data_info['ego_vehicle'] = self.ego_vehicle
        # if self.test_mode:
        #     data_info['ann_info'] = data_info['now']['eval_ann_info']
        # else:
        #     data_info['ann_info'] = data_info['now']['ann_info']




        data_info = {'infos': {}}
        data_info_now = self.load_frame_data(scene_idx, timestamp_idx)
        agents = data_info_now['agents']
        data_info_past = self.load_frame_data(scene_idx, timestamp_delay_idx)
        data_info['infos'][self.ego_vehicle] = data_info_now[self.ego_vehicle]
        for agent in agents:
            if agent != self.ego_vehicle:
                data_info['infos'][agent] = data_info_past[agent]
        position = dict()
        for agent in agents:
            position[agent] = data_info_now[agent]['position']
        data_info['agents'] = agents
        data_info['position'] = position
        data_info['time_delay'] = (timestamp_idx - timestamp_delay_idx) * self.frame_gap
        data_info['ego_vehicle'] = self.ego_vehicle
        if self.test_mode:
            data_info['ann_info'] = data_info_now['eval_ann_info']
        else:
            data_info['ann_info'] = data_info_now['ann_info']
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if 'scene_list' not in annotations or 'metainfo' not in annotations:
            raise ValueError('Annotation must have scene_list and metainfo '
                             'keys')
        metainfo = annotations['metainfo']
        self.frame_gap = metainfo['frame_gap']
        self.len = metainfo['scene_length']
        raw_scene_list = annotations['scene_list']
        ...
        scene_list = []
        for raw_scene_info in raw_scene_list:
            metainfo = raw_scene_info['metainfo']
            self.scene_len_list.append(metainfo['scene_length'])
            data_list = []
            for raw_data_info in raw_scene_info['data_list']:
                timestamp = int(raw_data_info['metainfo']['timestamp'])
                data_dict = raw_data_info['data_dict']
                data_dict['timestamp'] = timestamp
                data_dict['agents'] = raw_data_info['metainfo']['agents']
                data_info = self.parse_data_info(data_dict)
                data_info['timestamp'] = timestamp
                data_list.append(data_info)
            scene_list.append(data_list)

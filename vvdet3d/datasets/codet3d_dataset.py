...
from vvdet3d.registry import DATASETS
from vvdet3d.structures import get_box_type


@DATASETS.register_module()
class CoDet3DDataset(BaseDataset):
    """Base Class of 3D dataset.

    This is the base dataset of VVSim dataset.
    ...
    def parse_ann_info(self, info: dict) -> Union[dict, None]:
        ...
        instances = info['instances_local'][self.ego_vehicle]
        # empty gt
        if len(instances) == 0:
            return None
        else:
            ...
            for ann_name in keys:
                ...
                if 'label' in ann_name and ann_name != 'attr_label' and ann_name != 'label_name':
                    temp_anns = [
                ...
                if 'label_name' in ann_name:
                    # convert label_name to np.ndarray
                    temp_anns = np.array(temp_anns, dtype='<U20')
                elif 'label' in ann_name:
    ...
    def parse_data_info(self, info: dict) -> dict:
        ...
        for agent in agents:
            if self.modality['use_lidar']:
                if 'lidar_points' in info[agent]:
                    info[agent]['lidar_points']['lidar_path'] = \
                        osp.join(self.data_prefix.get('pts', ''),
                            info[agent]['lidar_points']['lidar_path'])

                    info[agent]['num_pts_feats'] = info[agent]['lidar_points']['num_pts_feats']
                    info[agent]['lidar_path'] = info[agent]['lidar_points']['lidar_path']
            if self.modality['use_camera']:
                for cam_id, img_info in info[agent]['images'].items():

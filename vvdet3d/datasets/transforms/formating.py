...
from vvdet3d.structures import BaseInstance3DBoxes, Det3DDataSample, PointData
from vvdet3d.structures.points import BasePoints
...
class Pack3DDetInputs(BaseTransform):
    ...
    def __init__(
        ...
        meta_keys: tuple = ('img_path', 'ori_shape', 'img_shape', 'lidar2img',
                            ...
                            'pcd_rotation_angle', 'lidar_path', 'lidar2ego',
                            'transformation_3d_flow', 'trans_mat', 'cam2ego',
                            'affine_aug', 'sweep_img_metas', 'ori_cam2img',
                            'cam2global', 'crop_offset', 'img_crop_offset',
                            'resize_img_shape', 'lidar2cam', 'ori_lidar2img',
                            'num_ref_frames', 'num_views', 'ego2global',
                            'axis_align_matrix', 'img_aug_matrix')
    ) -> None:
        self.keys = keys
    ...
    def pack_single_results(self, results: dict) -> dict:
        ...
        for agent in agents:
            # Format 3D data
            if 'points' in results['infos'][agent]:
                if isinstance(results['infos'][agent]['points'], BasePoints):
                    results['infos'][agent]['points'] = results['infos'][agent]['points'].tensor
            if 'img' in results['infos'][agent]:
                if isinstance(results['infos'][agent]['img'], list):
                    # process multiple imgs in single frame
                    imgs = np.stack(results['infos'][agent]['img'], axis=0)
                    if imgs.flags.c_contiguous:
                        imgs = to_tensor(imgs).permute(0, 3, 1, 2).contiguous()
                    else:
                        imgs = to_tensor(
                            np.ascontiguousarray(imgs.transpose(0, 3, 1, 2)))
                    results['infos'][agent]['img'] = imgs
                else:
                    img = results['infos'][agent]['img']
                    if len(img.shape) < 3:
                    ...
                    results['infos'][agent]['img'] = img

        for key in [
                'gt_bboxes', 'gt_bboxes_ignore', 'gt_labels',
                'gt_bboxes_labels', 'centers_2d', 'depths', 'gt_labels_3d'
        ]:
        ...
        data_metas = dict(ego_vehicle=results['ego_vehicle'], 
                          box_type_3d=results['box_type_3d'], 
                          box_mode_3d=results['box_mode_3d'],
                          sample_idx=results['sample_idx'])

        agents = results['agents']
        data_metas['infos'] = {}
        for agent in agents:
            data_metas['infos'][agent] = {}
            for key in self.meta_keys:
                if key in results['infos'][agent]:
                    data_metas['infos'][agent][key] = results['infos'][agent][key]
                elif 'images' in results['infos'][agent]:
                    if len(results['infos'][agent]['images'].keys()) == 1:
                        cam_type = list(results['infos'][agent]['images'].keys())[0]
                        # single-view image
                        if key in results['infos'][agent]['images'][cam_type]:
                            data_metas['infos'][agent][key] = results['infos'][agent]['images'][cam_type][key]
                    else:
                        # multi-view image
                        img_metas = []
                        cam_types = list(results['infos'][agent]['images'].keys())
                        for cam_type in cam_types:
                            if key in results['infos'][agent]['images'][cam_type]:
                                img_metas.append(results['infos'][agent]['images'][cam_type][key])
                        if len(img_metas) > 0:
                            data_metas['infos'][agent][key] = img_metas
                elif 'lidar_points' in results:
                    if key in results['infos'][agent]['lidar_points']:
                        data_metas['infos'][agent][key] = results['infos'][agent]['lidar_points'][key]
            ...
            for key in self.keys:
                if key in results['infos'][agent]:
                    if key in self.INPUTS_KEYS:
                        inputs['infos'][agent][key] = results['infos'][agent][key]
                elif key in results:
                    if key in self.INSTANCEDATA_3D_KEYS:

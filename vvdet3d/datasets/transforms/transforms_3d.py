...
from vvdet3d.models.task_modules import VoxelGenerator
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        for agent in agents:
            if 'drone' in agent.lower():
                continue
            idx = input_dict['infos'][agent]['points'].shuffle()
            idx = idx.numpy()
...
class ObjectRangeFilter(BaseTransform):
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if isinstance(input_dict['gt_bboxes_3d'],
            ...
            range_3d = self.pcd_range
        elif isinstance(input_dict['gt_bboxes_3d'], CameraInstance3DBoxes):
            raise NotImplementedError(
                'ObjectRangeFilter not implemented for CameraInstance3DBoxes')

        gt_bboxes_3d = input_dict['gt_bboxes_3d']
        gt_labels_3d = input_dict['gt_labels_3d']

        mask = gt_bboxes_3d.in_range_3d_without_self(range_3d)
...
class PointsRangeFilter(BaseTransform):
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        for agent in agents:
            if 'drone' in agent.lower():
                continue
            points = input_dict['infos'][agent]['points']
            points_mask = points.in_range_3d(self.pcd_range)
            clean_points = points[points_mask]
            input_dict['infos'][agent]['points'] = clean_points
        return input_dict

...
class EvalBox(abc.ABC):
    ...
    def __init__(self,
        ...
        assert type(sample_idx) == int, 'Error: sample_idx must be a int!'
...
class EvalBoxes:
    ...
    def __repr__(self):
        return "EvalBoxes with {} boxes across {} samples".format(len(self.all), len(self.sample_idxes))

    def __getitem__(self, item) -> List[EvalBoxType]:
        return self.boxes[item]

    def __eq__(self, other):
        if not set(self.sample_idxes) == set(other.sample_idxes):
            return False
        for token in self.sample_idxes:
    ...
    def all(self) -> List[EvalBoxType]:
        ...
        ab = []
        for sample_idx in self.sample_idxes:
            ab.extend(self[sample_idx])
        return ab

    @property
    def sample_idxes(self) -> List[str]:
        """ Returns a list of all keys. """
        return list(self.boxes.keys())

    def add_boxes(self, sample_idx: int, boxes: List[EvalBoxType]) -> None:
        """ Adds a list of boxes. """
        self.boxes[sample_idx].extend(boxes)
    ...
    def deserialize(cls, content: dict, box_cls):
        ...
        """
        eb = cls()
        for sample_idx, boxes in content.items():
            eb.add_boxes(int(sample_idx), [box_cls.deserialize(box) for box in boxes])
        return eb

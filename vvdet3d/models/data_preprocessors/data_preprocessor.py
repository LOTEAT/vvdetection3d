...
from vvdet3d.structures.det3d_data_sample import SampleList
from vvdet3d.utils import OptConfigType
...
class Det3DDataPreprocessor(DetDataPreprocessor):
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        ...
        """
        has_img = False
        agents = data['inputs']['infos'].keys()
        for agent in agents:
            if 'img' in data['inputs']['infos'][agent]:
                batch_pad_shape = self._get_pad_shape(data)
                has_img = True
                break
        data = self.collate_data(data, has_img)
        inputs, data_samples = data['inputs'], data['data_samples']
        batch_inputs = dict()
        
        batch_inputs['infos'] = {}
        for agent in agents:
            batch_inputs['infos'][agent] = {}
            if 'points' in inputs['infos'][agent]:
                batch_inputs['infos'][agent]['points'] = inputs['infos'][agent]['points']
                if self.voxel:
                    voxel_dict = self.voxelize(inputs['infos'][agent]['points'], data_samples)
                    batch_inputs['infos'][agent]['voxels'] = voxel_dict

        batch_input_shape = {}
        batch_input_shape['infos'] = {}
        agents = inputs['infos'].keys()
        if has_img:
            for agent in agents:
                imgs = inputs['infos'][agent]['imgs']
                batch_input_shape['infos'][agent] = tuple(imgs[0].size()[-2:])
        if has_img:
            if data_samples is not None:
                data_samples[0].set_metainfo({
                    'batch_input_shape': batch_input_shape,
                    'pad_shape': batch_pad_shape
                })
            ...
            for agent in agents:
                batch_inputs['infos'][agent]['imgs'] = inputs['infos'][agent]['imgs']
    ...
    def collate_data(self, data: dict, has_img: bool) -> dict:
        ...
        if has_img:
            if 'infos' in data['inputs']:
                agents = data['inputs']['infos'].keys()
                for agent in agents:
                    _batch_imgs = data['inputs']['infos'][agent]['img']
                    ...
                    if img_dim == 3:
                        ...
                        raise TypeError(
                            ...
                            f'{type(data)}: {data}')
                    data['inputs']['infos'][agent]['imgs'] = batch_imgs
    ...
    def _get_pad_shape(self, data: dict) -> List[Tuple[int, int]]:
        ...
        batch_pad_shape['infos'] = {}
        _batch_inputs['infos'] = {}
        agents = data['inputs']['infos'].keys()
        for agent in agents:
            _batch_inputs['infos'][agent] = data['inputs']['infos'][agent]['img']
            batch_pad_shape['infos'][agent] = []
            # Process data with `pseudo_collate`.
            if is_seq_of(_batch_inputs['infos'][agent], torch.Tensor):
                for ori_input in _batch_inputs['infos'][agent]:
                    if ori_input.dim() == 4:
                    ...
                    pad_w = int(
                        np.ceil(ori_input.shape[2] /
                                self.pad_size_divisor)) * self.pad_size_divisor
                    batch_pad_shape['infos'][agent].append((pad_h, pad_w))
            # Process data with `default_collate`.
            elif isinstance(_batch_inputs['infos'][agent], torch.Tensor):
                agent_shape = _batch_inputs['infos'][agent].shape
                assert _batch_inputs['infos'][agent].dim() == 4, (
                    'The input of `ImgDataPreprocessor` should be a NCHW tensor '
                    'or a list of tensor, but got a tensor with shape: '
                    f'{agent_shape}')
                pad_h = int(
                    np.ceil(_batch_inputs['infos'][agent].shape[1] /
                            self.pad_size_divisor)) * self.pad_size_divisor
                pad_w = int(
                    np.ceil(_batch_inputs['infos'][agent].shape[2] /
                            self.pad_size_divisor)) * self.pad_size_divisor
                batch_pad_shape['infos'][agent] = [(pad_h, pad_w)] * _batch_inputs['infos'][agent].shape[0]
            else:

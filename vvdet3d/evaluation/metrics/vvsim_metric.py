...
from vvdet3d.utils.vvsim_metrics.detection.data_classes import DetectionConfig
from nuscenes.utils.data_classes import Box as VVSimBox

from vvdet3d.models.layers import box3d_multiclass_nms
from vvdet3d.registry import METRICS
from vvdet3d.structures import (CameraInstance3DBoxes, LiDARInstance3DBoxes,
                                bbox3d2result, xywhr2xyxyr)
import json

@METRICS.register_module()
class VVSimMetric(BaseMetric):
    """VVSim evaluation metric.
    ...
    def __init__(self,
        ...
        self.default_prefix = 'VVSim metric'
        super(VVSimMetric, self).__init__(
        ...
        with open("./data/vvsim/config.json", 'r') as f:
            data = json.load(f)
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        self.data_infos = load(
            self.ann_file, backend_args=self.backend_args)['scene_list']
        self.scene_len_list = []
        for raw_scene_info in self.data_infos:
    ...
    def _evaluate_single(
        ...
        from vvdet3d.utils.vvsim_metrics import VVSimEval

        output_dir = osp.join(*osp.split(result_path)[:-1])
        data_info = pickle.load(open(self.ann_file, 'rb'))
        eval_set_map = {
            'v1.0-mini': 'mini_val',
            'v1.0-trainval': 'val',
        }
    ...
    def format_results(
        ...
        """Format the vvdet3d results to standard NuScenes json file.
    ...
    def _get_agent_info(self, sample_idx: int) -> dict:
        ...
        scene_info = self.data_infos[scene_idx]
    ...
    def _format_camera_bbox(self,
        ...
        for i, det in enumerate(mmengine.track_iter_progress(results)):
            ...
            annos = []
            boxes, attrs = output_to_vvsim_box(det)
            ...
            if sample_token in vvsim_annos:
                vvsim_annos[sample_token].extend(annos)
            else:
                vvsim_annos[sample_token] = annos
        ...
        res_path = osp.join(jsonfile_prefix, 'results_vvsim.json')
        print(f'Results writes to {res_path}')
        mmengine.dump(vvsim_results, res_path)
        return res_path
    ...
    def _format_lidar_bbox(self,
        ...
        for i, det in enumerate(mmengine.track_iter_progress(results)):
            ...
            boxes, attrs = output_to_vvsim_box(det)
            sample_idx = sample_idx_list[i]
            boxes = lidar_vvsim_box_to_global(self._get_agent_info(sample_idx),
                                             boxes, classes,
                                             self.eval_detection_configs)
            for i, box in enumerate(boxes):
                name = classes[box.label]
...
def output_to_vvsim_box(
        detection: dict) -> Tuple[List[VVSimBox], Union[np.ndarray, None]]:
    ...
    Returns:
        Tuple[List[:obj:`VVSimBox`], np.ndarray or None]: List of standard
        VVSimBoxes and attribute labels.
    """
    ...
    if isinstance(bbox3d, LiDARInstance3DBoxes):
        ...
        raise NotImplementedError(
            ...
            'to standard VVSimBoxes.')

    return box_list, attrs


def lidar_vvsim_box_to_global(
        info: dict, boxes: List[VVSimBox], classes: List[str],
        eval_configs: DetectionConfig) -> List[VVSimBox]:
    """Convert the box from ego to global coordinate.

    Args:
        info (dict): Info for a specific sample data, including the calibration
            information.
        boxes (List[:obj:`VVSimBox`]): List of predicted VVSimBoxes.
        classes (List[str]): Mapped classes in the evaluation.
        eval_configs (:obj:`DetectionConfig`): Evaluation configuration object.

    Returns:
        List[:obj:`DetectionConfig`]: List of standard VVSimBoxes in the
        global coordinate.
    """
    box_list = []
    for box in boxes:
...
def cam_nusc_box_to_global(
    ...
    Returns:
        Tuple[List[:obj:`VVSimBox`], List[int]]: List of standard
        VVSimBoxes in the global coordinate and attribute label.
    """
    box_list = []
    attr_list = []
...
def global_nusc_box_to_cam(info: dict, boxes: List[VVSimBox],
                           classes: List[str],
                           eval_configs: DetectionConfig) -> List[VVSimBox]:
    ...
    Returns:
        List[:obj:`VVSimBox`]: List of standard VVSimBoxes in camera
        coordinate.
    """
    box_list = []
    for box in boxes:
...
def nusc_box_to_cam_box3d(
    ...
    """Convert boxes from :obj:`VVSimBox` to :obj:`CameraInstance3DBoxes`.

    Args:
        boxes (:obj:`List[VVSimBox]`): List of predicted VVSimBoxes.
